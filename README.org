...
- Diagnostics: =pgsql-error= is the root condition for =pgsql-connection-error=, =pgsql-timeout=, =pgsql-protocol-error=, =pgsql-authentication-error=, and =pgsql-server-error=. =pgsql-error-fields= returns structured server fields, and each function in =pgsql-notice-functions= is called with a connection and a notice plist.
